...
for i, line in enumerate(lines):
    if line.startswith("Action ") and ":" in line and not line.lower().startswith("action pathway"):
        # We only want Action 1 to 6. If it's a repeat, skip.
        match = re.search(r"Action (\d+)", line)
        if match:
            act_id = int(match.group(1))
            # if we already have this action, skip parsing this line (it's the repeat at the end of doc)
    ...
    if current_pathway:
        ...
        if line_lower.startswith("why it is needed") or line_lower.startswith("why it is important"):
            ...
            # It could be length 16 or 19 depending on needed/important
            idx = line_lower.find(":")
            if idx != -1:

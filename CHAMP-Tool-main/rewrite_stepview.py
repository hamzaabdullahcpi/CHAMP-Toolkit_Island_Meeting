...
import SystemicPortfolioBlueprint from './SystemicPortfolioBlueprint';
...
export function StepView({ 
  ...
  const handleCloseModal = () => setSelectedExample(null);

  const hasLogic = Boolean(step.systemsLogic);
  const hasPathways = Boolean(step.pathways && step.pathways.length > 0);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24">
      ...
      {/* Master Top-Level Segmented Control (Only show if we have both Logic and Pathways) */}
      {hasLogic && hasPathways && (
        <div className="flex p-1.5 bg-surface border border-line rounded-lg max-w-[500px] mb-12 shadow-sm">
          <button 
            onClick={() => setTopLevelTab('logic')}
            className={`flex-1 py-3 px-4 text-[13px] font-bold uppercase tracking-widest rounded-md transition-all flex items-center justify-center gap-2 focus:outline-none ${
              topLevelTab === 'logic' 
                ? 'bg-paper shadow-sm border border-line/50 text-accent ring-1 ring-accent/10' 
                : 'text-ink-muted hover:text-ink hover:bg-black/5'
            }`}
          >
            <Settings size={16} className={topLevelTab === 'logic' ? 'text-accent' : 'opacity-60'} />
            Systems Logic
          </button>
          <button 
            onClick={() => setTopLevelTab('pathways')}
            className={`flex-1 py-3 px-4 text-[13px] font-bold uppercase tracking-widest rounded-md transition-all flex items-center justify-center gap-2 focus:outline-none ${
              topLevelTab === 'pathways' 
                ? 'bg-paper shadow-sm border border-line/50 text-accent ring-1 ring-accent/10' 
                : 'text-ink-muted hover:text-ink hover:bg-black/5'
            }`}
          >
            <GitMerge size={16} className={topLevelTab === 'pathways' ? 'text-accent' : 'opacity-60'} />
            Action Pathways
          </button>
        </div>
      )}
      ...
      <AnimatePresence mode="wait">
        ...
        {topLevelTab === 'logic' && hasLogic && (
          ...
          >
            <div className="p-8 md:p-10 bg-surface border border-line shadow-sm border-l-4 border-l-accent">
              <h2 className="font-heading text-2xl md:text-3xl font-medium text-ink mb-6">Systems Logic</h2>
              <p className="text-ink leading-relaxed text-[16px] md:text-[17px] font-light mb-10">
                {step.systemsLogic}
              </p>
              
              <div className="bg-paper p-4 border border-line shadow-sm rounded-md">
                <SystemicPortfolioBlueprint />
              </div>
            </div>
          </motion.div>
        )}

        {/* Action Pathways View */}
        {topLevelTab === 'pathways' && hasPathways && (
          <motion.div
            key="pathways-view"
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            {step.pathways.length > 1 && (
              <div className="mb-10">
                <h3 className="text-[13px] font-bold text-ink-muted uppercase tracking-widest mb-4">Select a Pathway</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {step.pathways.map((p: any, idx: number) => (
                    <button
                      key={idx}
                      onClick={() => setActivePathwayIndex(idx)}
                      className={`text-left p-5 border transition-all duration-300 rounded-sm flex flex-col justify-between min-h-[120px] focus:outline-none group ${
                        activePathwayIndex === idx
                          ? 'border-accent bg-accent/5 shadow-sm ring-1 ring-accent/20'
                          : 'border-line bg-surface hover:border-accent/50 hover:bg-paper hover:shadow-sm'
                      }`}
                    >
                      <h4 className={`font-heading text-lg leading-snug font-medium transition-colors ${activePathwayIndex === idx ? 'text-accent' : 'text-ink group-hover:text-ink'}`}>
                        {p.title || p.name}
                      </h4>
                      <div className={`mt-4 text-[12px] font-bold uppercase tracking-widest flex items-center gap-1 transition-colors ${activePathwayIndex === idx ? 'text-accent' : 'text-ink-muted group-hover:text-accent'}`}>
                        {activePathwayIndex === idx ? 'Active Pathway' : 'View Pathway'}
                        <ChevronRight size={14} className={activePathwayIndex === idx ? 'translate-x-1 transition-transform' : 'group-hover:translate-x-1 transition-transform'} />
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <PathwayCard 
                  pathway={step.pathways[activePathwayIndex]} 
      ...
      {/* Illustrative Example Modal (unchanged) */}
      <AnimatePresence>
        {selectedExample && (
          <motion.div
            initial={{ opacity: 0 }}

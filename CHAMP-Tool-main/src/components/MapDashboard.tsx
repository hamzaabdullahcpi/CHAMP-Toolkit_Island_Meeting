...
// In-memory module cache to avoid re-parsing TopoJSON / re-fetching on each navigation
let cachedGeoData: any = null;
let cachedFullOecdData: any = null;
const cdpCountryCache: Record<string, { count: number, costNeeded: number, projects: any[] }> = {};
const oecdCountryCache: Record<string, any> = {};
const sdgCountryCache: Record<string, any> = {};
const unPopCountryCache: Record<string, any> = {};
...
const normalizeName = (name: string) => {
  ...
  if (name === "Swaziland") return "Eswatini";
...
const champCountries = new Set([
  "Albania", "Andorra", "Antigua and Barbuda", "Armenia", "Australia", "Azerbaijan",
  "Bahamas", "Bangladesh", "Belgium", "Bhutan", "Bolivia", "Brazil", "Brunei Darussalam",
  "Bulgaria", "Burkina Faso", "Cabo Verde", "Canada", "Chad", "Chile", "Colombia",
  "Costa Rica", "Côte d’Ivoire", "Cote d'Ivoire", "Denmark", "Dominican Republic", "Dominican Rep.",
  "El Salvador", "Estonia", "Eswatini", "Ethiopia", "European Union", "Finland", "France",
  "Germany", "Ghana", "Guatemala", "Hungary", "Iceland", "Italy", "Jamaica", "Japan",
  "Jordan", "Kenya", "Kiribati", "Kyrgyzstan", "Lebanon", "Lesotho", "Mexico", "Moldova",
  "Mongolia", "Morocco", "Netherlands", "Nicaragua", "Nigeria", "North Macedonia", "Macedonia",
  "Norway", "Pakistan", "Palau", "Panama", "Papua New Guinea", "Paraguay", "Philippines",
  "Poland", "Portugal", "Republic of Korea", "South Korea", "Rwanda", "Saint Vincent and the Grenadines",
  "Serbia", "Seychelles", "Sierra Leone", "Sri Lanka", "Sweden", "Tunisia", "Türkiye", "Turkey",
  "Turkmenistan", "Ukraine", "United Arab Emirates", "United Kingdom", "United States", "United States of America", "Yemen"
]);
...
export default function MapDashboard({ stats, onNavigateToStep }: MapDashboardProps) {
  ...
  const [fullOecdData, setFullOecdData] = useState<any>(cachedFullOecdData);

  React.useEffect(() => {
    if (cachedFullOecdData) {
      setFullOecdData(cachedFullOecdData);
      return;
    }
    fetch(`${import.meta.env.BASE_URL}api/oecd.json`)
      ...
      .then(data => {
        if (data) {
          cachedFullOecdData = data;
          setFullOecdData(data);
        }
      })
      .catch(console.error);
  }, []);
  ...
  React.useEffect(() => {
    ...
    // Check caches first
    if (cdpCountryCache[normName]) {
      setCdpData({ count: cdpCountryCache[normName].count, costNeeded: cdpCountryCache[normName].costNeeded });
      setCdpProjects(cdpCountryCache[normName].projects);
    } else {
        ...
        try {
          ...
          while (hasMore && !isCancelled) {
            ...
            if (Array.isArray(data)) {
              ...
              if (data.length < limit || allData.length >= 2000) {
                hasMore = false;
              } else {
                offset += limit;
              }
          ...
          if (!isCancelled) {
            if (allData.length > 0) {
              ...
              cdpCountryCache[normName] = { count: allData.length, costNeeded: totalCost, projects: allData };
              setCdpData({ count: allData.length, costNeeded: totalCost });
              setCdpProjects(allData);
            } else {
              cdpCountryCache[normName] = { count: 0, costNeeded: 0, projects: [] };
    ...
    if (oecdCountryCache[normName] !== undefined) {
      setOecdData(oecdCountryCache[normName]);
    } else {
      const fetchOECD = async () => {
        ...
        try {
          ...
          if (!isCancelled) {
            const entry = (oecdJsonData && oecdJsonData[normName]) || null;
            oecdCountryCache[normName] = entry;
            setOecdData(entry);
          }
        } catch (err) {
          if (!isCancelled) setOecdData(null);
        } finally {
          if (!isCancelled) setLoadingOecd(false);
        }
      };
      fetchOECD();
    }

    if (sdgCountryCache[normName] !== undefined) {
      setSdgData(sdgCountryCache[normName]);
    } else {
      const fetchSDG = async () => {
        ...
        try {
          ...
          if (!isCancelled) {
            const entry = (sdgJsonData && sdgJsonData[normName]) || null;
            sdgCountryCache[normName] = entry;
            setSdgData(entry);
          }
        } catch (err) {
          if (!isCancelled) setSdgData(null);
        } finally {
          if (!isCancelled) setLoadingSdg(false);
        }
      };
      fetchSDG();
    }

    if (unPopCountryCache[normName] !== undefined) {
      setUnPopData(unPopCountryCache[normName]);
    } else {
      const fetchUnPop = async () => {
        ...
        try {
          ...
          if (!isCancelled) {
            const entry = (unPopJsonData && unPopJsonData[normName]) || null;
            unPopCountryCache[normName] = entry;
            setUnPopData(entry);
          }
        } catch (err) {
          if (!isCancelled) setUnPopData(null);
        } finally {
          if (!isCancelled) setLoadingUnPop(false);
        }
      };
      fetchUnPop();
    }
  ...
  React.useEffect(() => {
    ...
    fetch(geoUrl)
      ...
      .then((topology: any) => {
        ...
        const collection = { type: "FeatureCollection", features: newFeatures };
        cachedGeoData = collection;
  ...
  const getGeoColor = (geoName: string) => {
    ...
    if (activeTab === 'champ') {
      return champCountries.has(name) || champCountries.has(geoName) ? "#3B877E" : "#e2e8f0";
    }
  ...
  return (
    <div className="mb-24 relative">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between border-b border-line pb-6 gap-6">
        <h2 className="title-h2 max-w-lg">Snapshot: Multilevel Climate Action and Finance</h2>
      </div>
          ...
          return (
            <a 
              ...
              className={`flex flex-col justify-between p-8 relative transition-colors duration-300 ${
                isCorrespondingTab ? 'bg-paper shadow-[inset_0_-4px_0_0_#3B877E]' : 'bg-surface hover:bg-slate-50'
              }`}
            >
        ...
        <div className="w-full relative flex flex-col transition-all duration-300">
          ...
          <div className="absolute top-4 left-4 z-40 flex items-center gap-3">
            <button
              ...
              className="bg-white border border-line shadow-sm p-2 rounded-full hover:bg-slate-50 transition-colors text-slate-600 hover:text-[#3B877E] focus:outline-none"
                ...
                <div className="p-4 text-sm text-slate-600 space-y-3 font-light leading-relaxed">
                  {activeTab === 'champ' && (
                    <>
                      ...
                      <a href="https://www.cop28.com/en/cop28-uae-coalition-for-high-ambition-multilevel-partnerships-for-climate-action" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#3B877E] hover:underline font-medium mt-1">
                  ...
                  {activeTab === 'ndc' && (
                    <>
                      ...
                      <a href="https://unhabitat.org/sites/default/files/2025/02/urban_content_in_ndc_3.0._a_global_snapshot_updated-128-ndcs-1_1.pdf" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#3B877E] hover:underline font-medium mt-1">
                  ...
                  {activeTab === 'finance' && (
                    <>
                      ...
                      <a href="https://citiesclimatefinance.org/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#3B877E] hover:underline font-medium mt-1">
          ...
          <div className="absolute top-4 right-4 flex flex-col items-end gap-2 z-40 opacity-40 hover:opacity-100 transition-opacity duration-300">
            ...
            <div className="flex items-center gap-2">
                ...
                {showZoomSlider && (
                  ...
                  >
                    <input 
                      ...
                      className="w-full accent-[#3B877E] cursor-pointer"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              <button 
                ...
                className={`bg-white border text-ink border-line p-2 shadow-sm hover:bg-slate-100 transition-colors rounded-md ${showZoomSlider ? 'text-[#3B877E] bg-slate-50' : 'text-slate-600'}`}
              >
                <ZoomIn size={18} />
              </button>
            </div>
            ...
            <div className="bg-white border border-line shadow-sm flex flex-col rounded-md overflow-hidden">
              <button 
                ...
                className={`p-2 hover:bg-slate-100 transition-colors border-b border-line ${activeTool === 'default' ? 'bg-slate-100 text-[#3B877E]' : 'text-slate-600'}`}
              >
              ...
              <button 
                ...
                className={`p-2 hover:bg-slate-100 transition-colors ${activeTool === 'pan' ? 'bg-slate-100 text-[#3B877E]' : 'text-slate-600'}`}
              >
                <Hand size={18} />
              </button>
            </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {highNdcNonChamp.map(c => (
                               <button key={c} className="px-1.5 py-0.5 bg-[#E8F3F1] text-[#3B877E] border border-[#B9DED7] text-[10px] rounded-sm cursor-pointer hover:bg-[#DCEEEA] transition-colors" onClick={() => setSelectedCountry(c)}>{c}</button>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === 'champ' && (
                        <>
                          <div className="flex items-center gap-3">
                             <div className="w-3 h-3 flex-shrink-0 bg-[#3B877E]"></div>
                   ...
                   <div className="mb-6">
                      ...
                      {loadingSdg ? (
                        ...
                        <div className="grid grid-cols-1 gap-2">
                              <div className="bg-white p-2.5 border border-line rounded-sm shadow-sm flex items-center justify-between border-l-2 border-l-[#3B877E]/70">
                               <div>
                      ...
                      {loadingUnPop ? (
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="bg-white p-2.5 border border-line rounded-sm shadow-sm border-t-2 border-t-[#3B877E]/70">
                   ...
                   <div className="mb-6">
                      ...
                      {loadingOecd ? (
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-6">
                           ...
                           <div className="bg-white p-3 border border-line rounded-sm shadow-sm border-t-2 border-t-[#3B877E]">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
             ...
             >
                <div className="flex justify-between items-center p-6 border-b border-line bg-surface">
                   <div>
                     <h2 className="title-h2 mb-1">Reported CDP Projects</h2>
                     <p className="body-text-sm text-slate-500">Locally disclosed climate investments in {selectedCountry}</p>
                   </div>
